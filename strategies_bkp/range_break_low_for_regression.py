...
class RangeBreakDownStrategy(BaseStrategy):
    ...
    def evaluate(self):
        # Entry related
        if self.insight_book.curr_tpo is not None and self.insight_book.curr_tpo >= 7 and not self.trade_opp :#and fifteen_min_trend <= 0.2:
    ...
    def close_trade(self):
        if self.trade_open and not self.trade_close:
            last_candle = self.insight_book.last_tick
            if len(self.insight_book.market_data.items()) >= 355 and last_candle['low'] != self.insight_book.range['low']:
                self.trade_close = True
                self.trigger_exit_at_low()
