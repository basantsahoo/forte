from helper.utils import get_market_view

class TradeSet:
    def __init__(self, strategy, ts_id, triggers_per_signal, trade_set_info):
        self.strategy = strategy
        self.id = ts_id
        self.max_trades = triggers_per_signal
        self.trades = {}
    ...
    def trigger_entry(self):
        ...
        self.strategy.trigger_entry(self.id, list(self.trades.values()))

    def get_trades(self):
        next_trade = len(self.trades) + 1
        #Can contain None because it's inside expander
        triggers = [self.get_trade_details(trd_idx) for trd_idx in range(next_trade, next_trade + self.max_trades)]
        print('triggers===============', triggers)
        for trigger in triggers:
            #if leg is not None:
            self.triggers[trigger['seq']] = trigger
        return triggers

    def get_trade_details(self, trd_idx):
        trade_info = self.trade_set_info(trd_idx)
        return Trade(trade_info)

class Trade:
    def __init__(self, trade_set, trade_info):
        self.trade_set = trade_set
        self.trade_info = trade_info
        self.legs_count = len(trade_info['multi_leg'])
        self.multi_legs = [MultiLegGroup(multi_leg_info) for multi_leg_info in trade_info['multi_leg']]

class MultiLegGroup:
    def __init__(self, trade, multi_leg_info):
        self.trade = trade
        self.legs = [Leg(leg_info) for leg_info in multi_leg_info]
    ...
    def get_trade_leg_details(self, idx=1):
        ...
        market_view = get_market_view(instr)
        last_candle = self.trade.trade_set.strategy.get_last_tick(instr)
        if not last_candle:
            print('last_candle not found for', instr)
            trade_inst = self.strategy.get_closest_instrument(instr)
            instr = trade_inst
...
class IronCondor:
    def __init__(self, strategy, t_id, trade_inst, triggers_per_signal):
        ...
        self.max_triggers = triggers_per_signal
        self.triggers = {}
    ...
    def get_triggers(self):
        next_trigger = len(self.triggers) + 1
        #Can contain None because it's inside expander
        triggers = [self.get_trigger_details(trd_idx) for trd_idx in range(next_trigger, next_trigger + self.max_triggers)]
        print('triggers===============', triggers)
        for trigger in triggers:
            #if leg is not None:
            self.triggers[trigger['seq']] = trigger
    ...
    def get_trigger_details(self, idx=1):
        instr = self.trade_inst
        market_view = get_market_view(instr)
        last_candle = self.strategy.get_last_tick(instr)
        if not last_candle:
...
class Branches:
    def __init__(self, legs):
        self.legs = 2
        self.carry_forward_days = 0
        self.exit_at = 0
        self.force_exit_ts = 0
...
class TradeLeg:
    def __init__(self, **kwargs):
        ...
        self.exit_at = 0
        self.exit_at = 0
        self.spot_short_targets = []
        self.spot_short_stop_losses = []
        self.instr_targets = []
        self.instr_stop_losses = []
        self.adjust = {'pnl': 0.04, 'spot': 100}
        self.rolling_stop_losses = {'spot': 50, 'pnl': 0.04}
