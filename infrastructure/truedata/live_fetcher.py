...
nifty_expiry_dt = get_expiry_date(symbol=helper_utils.get_nse_index_symbol("NIFTY"))
nifty_expiry = nifty_expiry_dt.strftime('%y%m%d')

bank_nifty_expiry_dt = get_expiry_date(symbol=helper_utils.get_nse_index_symbol("BANKNIFTY"))
...
class TrueDataLiveFeed(socketio.ClientNamespace):
    ...
    def calculate_option_geeks(self,  option_chain, ticker):
        ...
        spot_price = self.spot_data[ticker]
        expiry_date = get_expiry_date(symbol=ticker)
        tim_to_expiry = helper_utils.get_time_to_expiry(expiry_date, ltt)/(3600*24*365) #(6.25*1+6*24)/(365*24*3600)
        r = 10 / 100
        # print(tim_to_expiry)
...
def restart_process():
    ...
    print("restart now")
    if ns is not None and ns.td_scoket is not None:
...
def start():
    ...
    scheduler.add_job(restart_process, 'cron', day_of_week='mon-fri', hour='11', minute='40')
