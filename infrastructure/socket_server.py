...
class LiveFeedNamespace(socketio.AsyncNamespace):
    ...
    async def on_td_price_feed(self, sid, feed):
        ...
        feed['min_volume'] = feed['turnover']
        self.option_processor.process_spot_data(feed)
        item = self.processor.process_input_data(feed)
        await self.emit('tick_data', {item['timestamp']: item}, room=item['symbol'])
        ley_list = ['symbol', 'ltp', "day_high", "day_low"]
        feed_small = {key: feed[key] for key in ley_list}
    ...
    async def on_join_options_feed_2(self, sid, ticker):
        print("JOIN OPTION FEED BY USER", helper_utils.root_symbol(ticker))
        spot_data = self.market_cache.get(helper_utils.root_symbol(ticker)) #self.option_processor.get_spot_data(helper_utils.root_symbol(ticker))
        print(spot_data)
        if spot_data is not None:
            spot_data['symbol'] = helper_utils.root_symbol(ticker)
            await self.emit('spot_data', spot_data, room=sid)
        #print(hist_data)
        ...
        if option_data:
            ...
            await self.emit('option_data', {'symbol':ticker, 'data':option_data}, room=sid)
...
class BacktestFeedNamespace(socketio.AsyncNamespace):
    def __init__(self, namespace=None):
        ...
        #self.pm = PortfolioManager(False)
        #self.processor.socket = self
    ...
    async def on_request_data(self, sid,ticker,  back_test_date):
        ...
        self.pm.trade_date = back_test_date
        ltp = self.processor.get_ltp(ticker)
        tick_size = get_tick_size(ltp)
        await self.emit('tick_size', tick_size, room=sid)
        await self.emit('ltp', ltp, room=sid)
    ...
    async def on_td_price_feed(self, feed):
        """
        print('td feed received')
        print(feed)
        """
...
async def refresh(ns):
    ...
    while True:
        ...
        #print(now.hour)
        #print(now.minute)
        if (now.hour == 8 and now.minute >= 45) or (now.hour == 9 and now.minute <= 5):
            ns.refresh()
