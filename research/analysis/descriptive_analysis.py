...
def group_wise_summary(report, df_i, grp):
    day_wise_df = df_i.groupby([grp]).agg({'realized_pnl': ['count', 'mean', 'min', 'max']})
    # day_wise_df = df_i.groupby(['strategy', 'support_ind',]).agg({'realized_pnl': ['count', 'mean', 'min', 'max']})
    day_wise_df.columns = ['count', 'pnl_avg', 'pnl_min', 'pnl_max']
    day_wise_df = day_wise_df.reset_index().round(2)
    aggregate_df = df_i.groupby(['strategy']).agg({'realized_pnl': ['count', 'mean', 'min', 'max']})
...
def add_strategy_details(report, df_i):
    ...
    winning_trades = df_i[df_i['realized_pnl'] > 0]
    losing_trades = df_i[df_i['realized_pnl'] < 0]
    winning_count = df_i[df_i['realized_pnl'] > 0]['realized_pnl'].count()
    winning_avg = round(df_i[df_i['realized_pnl'] > 0]['realized_pnl'].mean(), 2)
    losing_count = df_i[df_i['realized_pnl'] < 0]['realized_pnl'].count()
    losing_avg = round(df_i[df_i['realized_pnl'] < 0]['realized_pnl'].mean(), 2)
    winning_pct = round(winning_trades.shape[0] / (winning_trades.shape[0] + losing_trades.shape[0]) * 100, 1)
    print('winning trade count', df_i[df_i['realized_pnl'] > 0]['realized_pnl'].count())
    print('winning trade avg', round(df_i[df_i['realized_pnl'] > 0]['realized_pnl'].mean(), 2))
    print('losing trade count', df_i[df_i['realized_pnl'] < 0]['realized_pnl'].count())
    print('losing trade avg', round(df_i[df_i['realized_pnl'] < 0]['realized_pnl'].mean(), 2))
    ...
    with PdfPages(reports_dir +'descrpitive_analysis.pdf') as report:
        ...
        for root_strategy in root_strategies:
            ...
            for strategy in strategies:
                ...
                group_wise_summary(report, df_i, 'tpo')
