...
from backtest_2024.analysis import descriptive_analysis_multi_dataset, parameter_grid_search, classifier_train
import pandas as pd
import numpy as np
...
def load_time_results():
    df = pd.read_csv(reports_dir + 'high_inflex/' + 'option_market_impact_spot.csv')
    return df

def load_inflex_results():
    df = pd.read_csv(reports_dir + 'high_inflex/' + 'option_market_impact_spot_at_inflex.csv')
    return df
...
def scen_01():
    ...
    time_df = time_df[(time_df['near_call_oi_share'] <= 0.147) & (time_df['near_call_oi_share'] > 0)]
    #time_df = time_df[(time_df['pcr_minus_1'] >= 0.23) & (time_df['pcr_minus_1'] < 100)]
    time_df = time_df[(time_df['put_call_vol_scale_diff'] >= 1) & (time_df['put_call_vol_scale_diff'] < 1.5)]

    inflex_df = inflex_df[(inflex_df['near_call_oi_share'] <= 0.147) & (inflex_df['near_call_oi_share'] > 0)] #40
    #inflex_df = inflex_df[(inflex_df['pcr_minus_1'] >= 0.23) & (inflex_df['pcr_minus_1'] < 100)]
    inflex_df = inflex_df[(inflex_df['put_call_vol_scale_diff'] >= 1) & (inflex_df['put_call_vol_scale_diff'] < 1.5)]

    print("******* Time DF High analysis *****")
    basic_statistics(time_df)
    print("******* Inflex DF High analysis *****")
...
def scen_02():
    ...
    time_df = time_df[(time_df['pcr_minus_1'] >= 0.2) & (time_df['pcr_minus_1'] < 100)]
    time_df = time_df[(time_df['near_put_oi_share'] <= 0.118) & (time_df['near_put_oi_share'] > 0)]

    inflex_df = inflex_df[(inflex_df['pcr_minus_1'] >= 0.2) & (inflex_df['pcr_minus_1'] < 100)]
    inflex_df = inflex_df[(inflex_df['near_put_oi_share'] <= 0.118) & (inflex_df['near_put_oi_share'] > 0)]
...
def scen_03():
    ...
    time_df = time_df[(time_df['near_call_oi_share'] <= 0.147) & (time_df['near_call_oi_share'] > 0)]
    time_df = time_df[(time_df['near_call_volume_share_per_oi'] <= 50) & (time_df['near_call_volume_share_per_oi'] > 2.08)]
    # time_df = time_df[(time_df['pcr_minus_1'] >= 0.23) & (time_df['pcr_minus_1'] < 100)]
    #time_df = time_df[(time_df['put_call_vol_scale_diff'] < -1) & (time_df['put_call_vol_scale_diff'] > -100)]

    inflex_df = inflex_df[(inflex_df['near_call_oi_share'] <= 0.147) & (inflex_df['near_call_oi_share'] > 0)] #40
    inflex_df = inflex_df[(inflex_df['near_call_volume_share_per_oi'] <= 50) & (inflex_df['near_call_volume_share_per_oi'] > 2.08)]
    #inflex_df = inflex_df[(inflex_df['put_call_vol_scale_diff'] < -1) & (inflex_df['put_call_vol_scale_diff'] > -100)]
    # inflex_df = inflex_df[(inflex_df['pcr_minus_1'] >= 0.23) & (inflex_df['pcr_minus_1'] < 100)]
...
def scen_04():
    ...
    time_df = time_df[(time_df['near_call_oi_share'] <= 0.147) & (time_df['near_call_oi_share'] > 0)]
    time_df = time_df[(time_df['far_call_volume_share_per_oi'] <= 50) & (time_df['far_call_volume_share_per_oi'] > 1.3)]

    inflex_df = inflex_df[(inflex_df['near_call_oi_share'] <= 0.147) & (inflex_df['near_call_oi_share'] > 0)]  # 40
    inflex_df = inflex_df[(inflex_df['far_call_volume_share_per_oi'] <= 50) & (inflex_df['far_call_volume_share_per_oi'] > 1.3)]
