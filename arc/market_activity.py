...
class AssetActivityLog:
    ...
    def set_up(self):
        ...
        self.t_minus_1 = {'high':self.spot_book.yday_profile['high'], 'va_h_p':self.spot_book.yday_profile['va_h_p'],'poc_price':self.insight_book.yday_profile['poc_price'], 'va_l_p':self.insight_book.yday_profile['va_l_p'], 'low':self.insight_book.yday_profile['low'], 'open':self.insight_book.yday_profile['open'], 'close':self.insight_book.yday_profile['close']}
        self.t_minus_2 = {'high': self.spot_book.day_before_profile['high'],
                          'va_h_p': self.spot_book.day_before_profile['va_h_p'],
                          'poc_price': self.spot_book.day_before_profile['poc_price'],
                          'va_l_p': self.spot_book.day_before_profile['va_l_p'],
                          'low': self.spot_book.day_before_profile['low'],
                          'open': self.spot_book.day_before_profile['open'],
                          'close': self.spot_book.day_before_profile['close']}
        self.open_type = self.spot_book.state_generator.get_features()['open_type']
    ...
    def determine_day_open(self): ## this is definitive
        ...
        if open_low >= self.spot_book.yday_profile['high']:
            self.open_type = 'GAP_UP'
        elif open_high <= self.spot_book.yday_profile['low']:
            self.open_type = 'GAP_DOWN'
        elif open_low >= self.spot_book.yday_profile['va_h_p']:
            self.open_type = 'ABOVE_VA'
        elif open_high <= self.spot_book.yday_profile['va_l_p']:
    ...
    def determine_level_break(self, ts):
        for k in self.yday_level_breaks:
            if not self.yday_level_breaks[k]['value']:
                level_range = [self.spot_book.yday_profile[k] * (1 - 0.0015), self.spot_book.yday_profile[k] * (1 + 0.0015)]
                ol = get_overlap(level_range, [self.range['low'], self.range['high']])
                if ol > 0:
                    self.yday_level_breaks[k]['value'] = True
                    self.yday_level_breaks[k]['time'] = ts-self.spot_book.market_book.ib_periods[0]
        for k in self.day_before_level_breaks:
            if not self.day_before_level_breaks[k]['value']:
                level_range = [self.spot_book.day_before_profile[k] * (1 - 0.0015), self.spot_book.day_before_profile[k] * (1 + 0.0015)]
                ol = get_overlap(level_range, [self.range['low'], self.range['high']])
                if ol > 0:
                    self.day_before_level_breaks[k]['value'] = True
                    self.day_before_level_breaks[k]['time'] = ts-self.spot_book.market_book.ib_periods[0]
    ...
    def check_support(self, candle):
        support_ind = 0
        for support in self.spot_book.supports_to_watch:
    ...
    def check_resistance(self, candle):
        resistance_ind = 0
        for resistance in self.spot_book.resistances_to_watch:
    ...
    def candle_position_wrt_key_levels(self, candle):
        resp = {}
        yday_profile = {k: v for k, v in self.spot_book.yday_profile.items() if k in ('high', 'low', 'va_h_p', 'va_l_p', 'poc_price')}
        for (lvl, price) in yday_profile.items():
            resp['y_' + lvl] = self.check_level(candle, price)
        t_2_profile = {k: v for k, v in self.spot_book.day_before_profile.items() if k in ('high', 'low', 'va_h_p', 'va_l_p', 'poc_price')}
        for (lvl, price) in t_2_profile.items():
            resp['t_2_' + lvl] = self.check_level(candle, price)
        weekly_profile = {k: v for k, v in self.spot_book.weekly_pivots.items() if k not in ('open', 'close')}
